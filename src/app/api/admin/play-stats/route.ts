...
import { PlayRecord, UserPlayStat as BaseUserPlayStat } from '@/lib/types';

// 扩展 UserPlayStat 类型，包含 avatar 和 loginIp
interface UserPlayStat extends BaseUserPlayStat {
  avatar?: string;
  loginIp?: string;
}
  ...
  try {
    ...
    for (const user of allUsers) {
      try {
        ...
        // 出错的用户显示为空统计，但仍尝试获取IP
        let errorLoginIp = '获取失败';
        try {
          if (typeof (storage as any).getUserLoginIp === 'function') {
            const loginIp = await (storage as any).getUserLoginIp(
              user.username
            );
            errorLoginIp = loginIp || '暂无IP记录';
          }
        } catch (ipError) {
          console.error(`获取用户 ${user.username} 登录IP失败:`, ipError);
        }
