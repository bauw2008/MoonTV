...
function extractRecommendations(content: string) {
  ...
  // 使用正则表达式匹配电影推荐
  const moviePattern = /(\d+\.\s+)(.+?)\s*\((\d{4})\)\s*-\s*(.+?)\s*$/gm;
...
export const POST = AuthGuard.user(
  async (request: NextRequest, { user }: { user: any }) => {
    try {
      ...
      // 简化的双重控制：全局开关 + 用户权限
      const userConfig = adminConfig.UserConfig.Users.find(u => u.username === username);
      const hasPermission = (
        userConfig?.role === 'admin' || 
        userConfig?.role === 'owner' ||
        userConfig?.enabledApis?.includes('ai-recommend')
      );
      
      console.log('AI推荐双重控制检查:', {
        username,
        globalEnabled: adminConfig.AIRecommendConfig?.enabled,
        userHasPermission: hasPermission,
        canUse: adminConfig.AIRecommendConfig?.enabled && hasPermission
      });
      
      if (!adminConfig.AIRecommendConfig?.enabled) {
...
请始终保持专业和有用的态度，根据用户输入的内容类型提供相应的服务。`;
      ...
      let tokenLimit =
        max_tokens || max_completion_tokens || (aiConfig as any).maxTokens || 4096;
