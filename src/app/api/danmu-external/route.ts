...
export const GET = AuthGuard.user(
  async (request: NextRequest, { user }: { user: any }) => {
    try {
      ...
      // 如果直接提供了URL，使用它
      if (videoUrl) {
        platformUrls.push({ platform: 'direct', url: videoUrl });
      } else {
        // 优先从豆瓣页面提取链接
        if (doubanId) {
      ...
      allDanmu.forEach((danmu) => {
        ...
        const uniqueKey = `${Math.round(danmu.time * 10) / 10}_${danmu.text}`;
      ...
      console.log(
        `📊 弹幕统计: 原始${allDanmu.length}条 -> 去重后${uniqueDanmu.length}条`,
      );
