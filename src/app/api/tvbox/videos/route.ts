...
import { getVideosByCategory } from '@/lib/Analysis';
...
function buildCategoryStructure(categories: any[]): any {
  ...
  // 获取所有二级分类 (type_pid !== 0)
  structure.secondary_categories = categories.filter(cat => cat.type_pid !== 0);
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!site) return NextResponse.json({ error: '视频源不存在' }, { status: 404 });
    ...
    try {
      const categoryResult = await getVideosByCategory(site, category || undefined, page);
      ...
      // 如果有分类参数，手动过滤结果
      if (category) {
        results = searchResults.filter((item: any) => 
