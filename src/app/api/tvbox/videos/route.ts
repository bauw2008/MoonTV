...
export async function GET(request: NextRequest) {
  try {
      ...
      try {
        ...
        if (
          ...
          if (yellowWords && yellowWords.length > 0) {
            ...
            // 检查用户是否需要应用18+过滤
            const userConfig = tvboxConfig.UserConfig.Users?.find(u => u.username === username);
            ...
            if (!tvboxConfig.SiteConfig.DisableYellowFilter) {
              ...
              } else if (userConfig?.tags && userConfig.tags.length > 0 && tvboxConfig.UserConfig.Tags) {
                for (const tagName of userConfig.tags) {
                  const tagConfig = (tvboxConfig.UserConfig.Tags as any)?.find((t: any) => t.name === tagName);
            ...
            if (shouldFilter) {
              ...
              videosWithSourceName = videosWithSourceName.filter(
                (item: any) => !yellowWords.some((word: string) => {
                  const title = (item.title || '').toLowerCase();
                  const typeName = (item.type_name || '').toLowerCase();
                  return title.includes(word.toLowerCase()) || typeName.includes(word.toLowerCase());
                }),
              );
            }
          }
    ...
    // 从API获取数据
    try {
      const { results, pageCount } = await getVideosByCategory(site, category, page);
      ...
      try {
        ...
        if (categoryResponse.ok) {
          ...
          if (categoryData.class && Array.isArray(categoryData.class)) {
            ...
            const categoryMap = categoryData.class.reduce((map, cat) => {
              map[cat.type_id] = cat;
              return map;
            ...
            if (yellowWords && yellowWords.length > 0) {
              ...
              if (shouldFilter) {
                finalCategoryStructure.primary_categories = finalCategoryStructure.primary_categories.filter((category) => {
                  ...
                  return !yellowWords.some((word) => categoryName.includes(word));
                });
                ...
                Object.entries(finalCategoryStructure.category_map).forEach(([id, category]) => {
                  const categoryName = (category as any).type_name || '';
      ...
      if (yellowWords && yellowWords.length > 0) {
        ...
        if (shouldFilter) {
          resultsWithSourceName = resultsWithSourceName.filter((item: any) => {
            ...
            return !yellowWords.some(word => combinedText.includes(word.toLowerCase()));
          });
        }
      }
      ...
      // 缓存结果
      try {
        await setTVBoxVideoCache(source, {
