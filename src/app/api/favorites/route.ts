...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (key) {
      ...
      await db.deleteFavorite(authInfo.username, source, id);
    } else {
      // 清空全部
      const all = await db.getAllFavorites(authInfo.username);
      await Promise.all(
        Object.keys(all).map(async (k) => {
          const [s, i] = k.split('+');
          if (s && i) await db.deleteFavorite(authInfo.username, s, i);
        }),
      );
    }
