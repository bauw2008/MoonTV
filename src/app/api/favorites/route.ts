  ...
  async (request: NextRequest, { user }: { user: any }) => {
    try {
      ...
      if (key) {
        ...
        const fav = await db.getFavorite(user.username, source, id);
        return NextResponse.json(fav, { status: 200 });
      }

      // 查询全部收藏
      const favorites = await db.getAllFavorites(user.username);
...
export const POST = AuthGuard.user(
  async (request: NextRequest, { user }: { user: any }) => {
    try {
      ...
      await db.saveFavorite(user.username, source, id, finalFavorite);
