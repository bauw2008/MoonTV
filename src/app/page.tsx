  ...
  const updateFavoriteItems = async (allFavorites: Record<string, any>) => {
    ...
    const sorted = Object.entries(allFavorites)
      ...
      .map(([key, fav]) => {
        ...
        // 查找对应的播放记录，获取当前集数和最新的总集数
        const playRecord = allPlayRecords[key];
        const currentEpisode = playRecord?.index;

        // 优先使用播放记录中的总集数（动态更新），如果没有则使用收藏时的总集数
        const totalEpisodes =
          playRecord?.total_episodes || fav.total_episodes || 1;
        ...
        return {
          ...
          year: fav.year,
          poster: fav.cover,
          episodes: totalEpisodes, // 使用动态更新的总集数
          ...
          <div className='flex flex-row items-center justify-center gap-3 sm:w-auto'>
            ...
            {aiEnabled && (
              ...
              >
                ...
                <Brain className='h-3.5 w-3.5 relative z-10 group-hover:rotate-12 transition-transform duration-300' />
                <span className='relative z-10 text-sm'>AI推荐</span>
              </button>
            )}
          </div>
        </div>
