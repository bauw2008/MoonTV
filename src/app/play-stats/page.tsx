...
const PlayStatsPage: React.FC = () => {
  ...
  const { state: authInfo } = useAuth();
  const { user, loading: authLoading } = authInfo;
  ...
  useEffect(() => {
    ...
    // play-stats 页面普通用户也可以访问（查看个人播放记录）
    // 管理员可以看到全站统计，普通用户只能看个人统计
  ...
  }, [isAuthenticated, isAdmin]); // ✅ 只在认证状态和管理员权限变化时调用
