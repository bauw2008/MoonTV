...
import { api } from '@/lib/auth/client/fetch';
    ...
    const fetchSourceDetail = async (
        ...
        id: string,
          ...
          try {
            ...
            const detailData = detailResponse as SearchResult;
                ...
                if (!data || Object.keys(data).length === 0) {
                  ...
                  if (typeof window !== 'undefined') {
                    import('@/components/Toast').then(({ ToastManager }) => {
                      ToastManager?.warning('视频加载异常，正在尝试恢复...');
                    });
                  }
                  ...
                  if (hls && video) {
                    try {
                      ...
                      if (typeof window !== 'undefined') {
                        import('@/components/Toast').then(({ ToastManager }) => {
                          ToastManager?.error('视频加载失败，请刷新页面重试');
                        });
                      }
                    }
                  }
                ...
                if (data.fatal) {
                  switch (data.type) {
                    case Hls.ErrorTypes.NETWORK_ERROR:
                      ...
                      if (typeof window !== 'undefined') {
                        import('@/components/Toast').then(({ ToastManager }) => {
                          ToastManager?.warning('网络不稳定，正在尝试恢复播放...');
                        });
                      }
                      try {
                        hls.startLoad();
                        ...
                        if (typeof window !== 'undefined') {
                          import('@/components/Toast').then(({ ToastManager }) => {
                            ToastManager?.error('网络连接失败，请检查网络后刷新页面');
                          });
                        }
                        hls.destroy();
                      }
                    ...
                    case Hls.ErrorTypes.MEDIA_ERROR:
                      ...
                      if (typeof window !== 'undefined') {
                        import('@/components/Toast').then(({ ToastManager }) => {
                          ToastManager?.warning('视频解码异常，正在尝试恢复...');
                        });
                      }
                      try {
                        ...
                        if (typeof window !== 'undefined') {
                          import('@/components/Toast').then(({ ToastManager }) => {
                            ToastManager?.error('视频格式不支持，请尝试其他视频源');
                          });
                        }
                        hls.destroy();
                      }
