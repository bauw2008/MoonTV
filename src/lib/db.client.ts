...
import { api } from './auth/client/fetch';
...
// ---- 收藏类型 ----
// 注意：Favorite 类型已从 './types' 导入，此处不再重复定义
...
// ---- 常量 ----
const PLAY_RECORDS_KEY = 'vidora_play_records';
const FAVORITES_KEY = 'vidora_favorites';
const SEARCH_HISTORY_KEY = 'vidora_search_history';
const USER_STATS_KEY = 'vidora_user_stats'; // 添加用户统计数据存储键

// 缓存相关常量
const CACHE_PREFIX = 'vidora_cache_';
...
class HybridCacheManager {
  ...
  private clearAllCache(): void {
    ...
    keys.forEach((key) => {
      if (key.startsWith('vidora_cache_')) {
  ...
  getStoredAuthInfo(): {
    ...
    const username = this.getCurrentUsername();
    return {
      hasPlayRecords: !!this.getCachedPlayRecords(),
      hasFavorites: !!this.getCachedFavorites(),
      hasSearchHistory: !!this.getCachedSearchHistory(),
      hasUserStats: !!this.getCachedUserStats(),
      username,
    };
  }
...
// ---- 工具函数 ----
/**
 * 通用的 fetch 函数，使用新的认证框架
 */
async function fetchFromApi<T>(path: string): Promise<T> {
  const { api } = await import('@/lib/auth/client/fetch');
  ...
  if (!hasWatchedBeyondOriginal || !hasSignificantProgress) {
    console.log(
      `✗ 不更新原始集数: ${existingRecord.title} - 观看第${
        ...
        hasWatchedBeyondOriginal ? '观看时间不足' : '未超过原始集数'
      })`,
    );
    return {
      shouldUpdate: false,
...
export async function savePlayRecord(
  ...
  if (STORAGE_TYPE !== 'localstorage') {
    ...
    // 异步同步到数据库
    try {
      await api.post('/api/playrecords', { key, record });
...
export async function deletePlayRecord(
  ...
  if (STORAGE_TYPE !== 'localstorage') {
    ...
    // 异步同步到数据库
    try {
      await api.delete(`/api/playrecords?key=${encodeURIComponent(key)}`);
    } catch (err) {
      await handleDatabaseOperationFailure('playRecords', err);
...
export async function addSearchHistory(keyword: string): Promise<void> {
  ...
  if (STORAGE_TYPE !== 'localstorage') {
    ...
    // 异步同步到数据库
    try {
      await api.post('/api/searchhistory', { keyword: trimmed });
    } catch (err) {
...
export async function saveFavorite(
  ...
  if (STORAGE_TYPE !== 'localstorage') {
    ...
    try {
      ...
      await api.post('/api/favorites', { key, favorite });
    } catch (err) {
...
export async function deleteFavorite(
  ...
  if (STORAGE_TYPE !== 'localstorage') {
    ...
    // 异步同步到数据库
    try {
      await api.delete(`/api/favorites?key=${encodeURIComponent(key)}`);
    } catch (err) {
      await handleDatabaseOperationFailure('favorites', err);
...
export function getCacheStatus(): {
  ...
  const authInfo = cacheManager.getStoredAuthInfo();
  return {
...
async function calculateStatsFromLocalData(): Promise<UserStats> {
  try {
    ...
    if (records.length === 0) {
      return {
        username: cacheManager.getStoredAuthInfo()?.username || 'unknown',
        totalWatchTime: 0,
        totalPlays: 0,
        lastPlayTime: 0,
        recentRecords: [],
    ...
    const stats: UserStats = {
      ...
      firstWatchDate,
      lastUpdateTime: Date.now(),
      registrationDays: calculateRegistrationDays(firstWatchDate),
      lastLoginTime: lastPlayTime,
    };
...
export async function updateUserStats(record: PlayRecord): Promise<void> {
  ...
  try {
    ...
    if (watchTimeIncrement > 0) {
      ...
      if (STORAGE_TYPE !== 'localstorage') {
        try {
          const responseData = await api.post('/api/user/my-stats', {
...
export async function clearUserStats(): Promise<void> {
  try {
    if (STORAGE_TYPE !== 'localstorage') {
      // 从服务器清除
      await api.delete('/api/user/my-stats');
