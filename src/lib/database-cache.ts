  ...
  static async getKVRocksCacheStats() {
    ...
    try {
      ...
      allCacheKeys.forEach((fullKey: string, idx: number) => {
        ...
        } else if (key.startsWith('tvbox-') || key.startsWith('tvbox:')) {
          stats.tvbox.count++;
  ...
  static async getSimpleCacheStats() {
    ...
    if (redisStats) {
      return {
        ...
        formattedSizes: {
          ...
          tvbox: formatBytes(redisStats.tvbox.size),
    ...
    return {
      ...
      formattedSizes: {
        ...
        tvbox: formatBytes(stats.tvbox.size),
    ...
    try {
      switch (type) {
        ...
        case 'tvbox':
          ...
          if (typeof localStorage !== 'undefined') {
            const keys = Object.keys(localStorage).filter((key) =>
              key.startsWith('tvbox-') || key.startsWith('tvbox:')
            );
            keys.forEach((key) => {
              localStorage.removeItem(key);
              clearedCount++;
            });
            console.log(`🗑️ localStorage中清理了 ${keys.length} 个TVBox缓存项`);
          }
