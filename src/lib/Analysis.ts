import { API_CONFIG, type ApiSite } from '@/lib/config'; // 从 config 导入 ApiSite
import { cleanHtmlTags } from '@/lib/utils';
import type { SearchResult } from '@/lib/types'; // 从 types 导入 SearchResult
...
export async function getVideosByCategory(
  ...
  try {
    ...
    // 过滤掉集数为 0 的结果
    const filteredResults = results.filter((result: SearchResult) => result.episodes.length > 0);
