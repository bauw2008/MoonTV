...
import { getDetailedWatchingUpdates } from '@/lib/watching-updates';
...
const TopNav = ({ activePath: _activePath = '/' }: TopNavProps) => {
  ...
  const [notifications, setNotifications] = useState({
    ...
    watchingUpdates: { hasUpdates: false, count: 0, series: [] },
  ...
  // 获取追剧更新
  useEffect(() => {
    const fetchWatchingUpdates = async () => {
      try {
        const enableWatchingUpdates = localStorage.getItem(
          'enableWatchingUpdates',
        );
        if (enableWatchingUpdates !== 'true') {
          return;
        }
        ...
        if (updates) {
          setNotifications((prev) => ({
            ...
            watchingUpdates: {
              ...
              count: updates.updatedCount || 0,
              series: updates.updatedSeries || [],
            },
          }));
        }
      } catch (_error) {
    ...
    fetchWatchingUpdates();
    // 每5分钟检查一次追剧更新
    const interval = setInterval(fetchWatchingUpdates, 5 * 60 * 1000);
            ...
            if (state.user?.role === 'admin' || state.user?.role === 'owner') {
              ...
              comment.replies.forEach((reply: Comment) => {
                ...
                if (
                  ...
                  reply.username !== state.user?.username
                ) {
                  unreadCount++;
                }
              });
  ...
  const hasWatchingUpdates = notifications.watchingUpdates.hasUpdates;
      ...
      <div className='fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[100] p-4'>
        <div className='bg-white/95 dark:bg-gray-900/95 backdrop-blur-xl rounded-2xl shadow-2xl max-w-md w-full p-6 relative max-h-[80vh] overflow-y-auto border border-gray-200/50 dark:border-gray-700/50'>
          ...
          <div className='space-y-3'>
            ...
            {/* 追剧更新提醒 */}
            {notifications.watchingUpdates.hasUpdates && (
              <div className='group relative p-4 bg-gradient-to-r from-green-50 to-teal-50 dark:from-green-900/20 dark:to-teal-900/20 rounded-xl border border-green-200/50 dark:border-teal-800/50 shadow-sm hover:shadow-md transition-all duration-300'>
                <div className='flex items-start space-x-4'>
                  ...
                  <div className='flex-1'>
                    ...
                    <p className='text-sm text-gray-600 dark:text-gray-400 mt-1'>
                      有 {notifications.watchingUpdates.count} 部剧集有新内容
                    </p>
                    {notifications.watchingUpdates.series.length > 0 && (
                      <div className='mt-3 space-y-2'>
                        {notifications.watchingUpdates.series
                          .slice(0, 3)
                          .map((series, index) => (
                            <div
                              key={index}
                              className='flex items-center gap-3 p-2 bg-white/50 dark:bg-gray-800/50 rounded-lg cursor-pointer hover:bg-white/80 dark:hover:bg-gray-800/80 transition-all duration-200'
                            >
                                ...
                                <img
                                  ...
                                  onError={(e) => {
                                    (e.target as HTMLImageElement).src =
                                      '/api/image-proxy?url=' +
                                      encodeURIComponent(series.cover || '');
                                  }}
                                />
                              </div>
                  ...
                  {/* 追剧更新徽章 */}
                  <span className='absolute top-2 right-2 px-2 py-0.5 text-[10px] font-medium text-white bg-green-500 rounded-full'>
                    {notifications.watchingUpdates.count > 99
                      ? '99+'
                      : notifications.watchingUpdates.count}
                  </span>
                </div>
              </div>
            )}
