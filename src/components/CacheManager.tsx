...
export default function CacheManager() {
  ...
  const clearAllCache = async () => {
    if (
      !confirm(
        '⚠️ 确定要清理所有缓存吗？这将清除豆瓣、弹幕、网盘搜索、YouTube搜索等所有缓存数据。'
      )
    ) {
      return;
    }
  ...
  return (
    <div className='space-y-6'>
      ...
      <div className='flex items-center justify-end'>
        <div className='flex items-center space-x-2'>
          ...
          <button
            onClick={fetchStats}
            disabled={loading}
            className='inline-flex items-center px-3 py-1.5 text-sm font-medium text-gray-700 dark:text-gray-300 bg-transparent dark:bg-transparent border border-gray-300 dark:border-gray-600 rounded-md hover:bg-gray-50 dark:hover:bg-gray-600 disabled:opacity-50'
          >
            <ArrowPathIcon
        ...
        <div className='grid grid-cols-1 lg:grid-cols-2 gap-6'>
          {CACHE_TYPES.map((cacheType) => {
            ...
            return (
              <div
                key={cacheType.key}
                className='bg-transparent dark:bg-transparent rounded-lg border border-gray-200 dark:border-gray-700 p-6'
              >
                ...
                <div className='grid grid-cols-2 gap-4 mb-4'>
                  <div className='text-center p-3 bg-transparent dark:bg-transparent rounded-lg'>
      ...
      {/* 批量操作 */}
      {stats && (
        <div className='bg-transparent dark:bg-transparent rounded-lg border border-gray-200 dark:border-gray-700 p-6'>
