...
function LiveConfigContent() {
  ...
  const loadConfig = async () => {
    try {
      const data = await withLoading('loadLiveConfig', () =>
  ...
  const handleAddLiveSource = async () => {
    if (!newLiveSource.name || !newLiveSource.key || !newLiveSource.url) {
      if (typeof window !== 'undefined') {
        import('@/components/Toast').then(({ ToastManager }) => {
          ToastManager?.error('请填写必要信息（名称、标识、URL）');
        });
      }
      return;
    }
  ...
  const handleEditLiveSource = async () => {
    if (!editingLiveSource?.name || !editingLiveSource.url) {
      if (typeof window !== 'undefined') {
        import('@/components/Toast').then(({ ToastManager }) => {
          ToastManager?.error('请填写必要信息（名称、URL）');
        });
      }
      return;
    }
      ...
      <div className='space-y-6'>
        ...
        <div className='flex flex-wrap gap-2'>
          ...
          <button
            ...
            disabled={isRefreshing || loading.refreshLiveSources}
            className='flex items-center space-x-2 px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors disabled:opacity-50'
          >
            <RefreshCw
              className={
                isRefreshing || loading.refreshLiveSources ? 'animate-spin' : ''
              }
              size={16}
            />
            <span>
              {isRefreshing || loading.refreshLiveSources
                ? '刷新中...'
                : '刷新直播源'}
            </span>
          </button>
          ...
          {orderChanged && (
            <button
              ...
              disabled={loading.saveLiveSourceOrder}
            ...
            >
              <Save size={16} />
              <span>
                {loading.saveLiveSourceOrder ? '保存中...' : '保存顺序'}
              </span>
            </button>
          )}
        </div>
          ...
          <div className='bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-700 rounded-lg p-6'>
            ...
            <div className='flex justify-end space-x-2 mt-4'>
              ...
              <button
                ...
                disabled={loading.addLiveSource}
                className='px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50'
              >
                {loading.addLiveSource ? '添加中...' : '添加'}
              </button>
            </div>
          </div>
        )}
