...
export function CacheManager() {
  ...
  const fetchStats = async () => {
    try {
      setError(null);
      const response = await withLoading('fetchCacheStats', async () => {
  ...
  const clearCache = async (type: string) => {
    ...
    try {
      ...
      const response = await withLoading(`clearCache_${type}`, async () => {
      ...
      if (response.success) {
        ...
        if (typeof window !== 'undefined') {
          import('@/components/Toast').then(({ ToastManager }) => {
            ToastManager?.success(`${typeName}缓存清理成功`);
          });
        }
      } else {
        throw new Error(response.error);
      ...
      <div className='space-y-6'>
        ...
        <div className='flex items-center justify-end'>
          <div className='flex items-center space-x-2'>
            ...
            <button
              onClick={fetchStats}
              disabled={loading.fetchCacheStats}
            ...
            >
              <RefreshCw
                className={`h-4 w-4 mr-1.5 ${loading.fetchCacheStats ? 'animate-spin' : ''}`}
              />
              刷新
            </button>
          </div>
        ...
        {stats && (
          <div className='bg-gradient-to-r from-blue-500 to-blue-600 rounded-lg p-6 text-white'>
            <div className='grid grid-cols-1 md:grid-cols-3 gap-4'>
              ...
              <div className='text-center'>
                <div className='text-3xl font-bold'>{CACHE_TYPES.length}</div>
