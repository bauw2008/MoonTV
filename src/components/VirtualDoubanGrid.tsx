...
interface VirtualDoubanGridProps {
  ...
  // 是否来自番组计划
  isBangumi?: boolean;
  ref?: React.Ref<VirtualDoubanGridRef>;
}
...
export const VirtualDoubanGrid = ({
  doubanData,
  hasMore,
  isLoadingMore,
  onLoadMore,
  ...
  const lastPrimarySelectionRef = useRef(primarySelection);
  ...
  // 重置可见项目数量（仅当类型或筛选条件改变时）
  useEffect(() => {
    // 检查类型或筛选条件是否真正改变
    const typeChanged = lastTypeRef.current !== type;
    const selectionChanged =
      lastPrimarySelectionRef.current !== primarySelection;
    ...
    // 注意：doubanData 不在依赖数组中，避免数据更新时重复触发
  ...
  // 加载更多项目
  const loadMoreVirtualItems = () => {
    if (isVirtualLoadingMore || !hasNextVirtualPage) {
      return;
    }
