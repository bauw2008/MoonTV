...
export const UserMenu: React.FC = () => {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined' && authInfo?.username) {
            ...
            comments.forEach((comment: any) => {
              ...
              if (authInfo.role === 'admin' || authInfo.role === 'owner') {
                ...
                if (
                  ...
                  comment.username !== authInfo.username
                ) {
                  unreadCount++;
                }
                ...
                comment.replies.forEach((reply: any) => {
                  ...
                  if (
                    ...
                    reply.username !== authInfo.username
                  ) {
                    unreadCount++;
                  }
                });
                ...
                // 如果是普通用户，只检查自己的留言回复
                if (comment.username === authInfo.username) {
                  comment.replies.forEach((reply: any) => {
                    // 如果回复时间在最后查看之后，则为未读
      ...
      // 每30秒检查一次待审核用户数量
      const pendingUsersInterval = setInterval(fetchPendingUsersCount, 30000);
      return () => {
  ...
  const handleMessageBoard = () => {
    ...
    if (typeof window !== 'undefined' && authInfo?.username) {
      const currentTime = Date.now();
      localStorage.setItem('lastViewedMessages', currentTime.toString());
