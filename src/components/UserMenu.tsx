...
export const UserMenu: React.FC = () => {
  ...
  const { state, logout } = useAuth(); // 使用新的认证Hook
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      if (state.user?.username) {
        fetchUserAvatar(state.user.username);
      }
  ...
  }, [state.user, storageType]);

  // 获取留言回复信息和待审核用户数量
  useEffect(() => {
    if (typeof window !== 'undefined' && state.user?.username) {
  ...
  const handleMenuClick = async () => {
    ...
    if (willOpen && state.user?.username && storageType !== 'localstorage') {
      try {
        // 暂时清除缓存时间，强制检查一次
        const lastCheckTime = localStorage.getItem('vidora_last_update_check');
        ...
        // 恢复缓存时间（如果之前有的话）
        if (lastCheckTime) {
          localStorage.setItem('vidora_last_update_check', lastCheckTime);
        }
  ...
  // 追剧提醒开关处理函数
  const handleEnableWatchingUpdatesToggle = (value: boolean) => {
    setEnableWatchingUpdates(value);
    if (typeof window !== 'undefined') {
      localStorage.setItem('enableWatchingUpdates', JSON.stringify(value));
    }
    // 如果关闭提醒，同时清除未读状态
    if (!value) {
      setHasUnreadUpdates(false);
  ...
  const handleConfirmCrop = async () => {
    if (!completedCrop || !imageRef.current || !state.user?.username) {
      return;
    }
  ...
  const handleResetSettings = () => {
    ...
    if (typeof window !== 'undefined') {
      ...
      localStorage.setItem('enableWatchingUpdates', JSON.stringify(true));
    }
  };
  ...
  const showChangePassword =
    state.user?.role !== 'owner' && storageType !== 'localstorage';

  // 检查是否显示播放统计按钮（所有登录用户，且非localstorage存储）
  const showPlayStats = state.user?.username && storageType !== 'localstorage';

  // 检查是否显示更新提醒按钮（登录用户且非localstorage存储就显示）
  const showWatchingUpdates =
    state.user?.username && storageType !== 'localstorage';
              ...
              <label className='flex items-center cursor-pointer'>
                <div className='relative'>
                  <input
                    type='checkbox'
                    className='sr-only peer'
                    checked={enableWatchingUpdates}
                    onChange={(e) =>
                      handleEnableWatchingUpdatesToggle(e.target.checked)
                    }
                  />
                  <div className='w-11 h-6 bg-gray-300 rounded-full peer-checked:bg-green-500 transition-colors dark:bg-gray-600'></div>
                  <div className='absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full transition-transform peer-checked:translate-x-5'></div>
    ...
    <>
      <div className='relative'>
        ...
        {(updateStatus === UpdateStatus.HAS_UPDATE ||
          (enableWatchingUpdates && hasUnreadUpdates && totalUpdates > 0)) && (
