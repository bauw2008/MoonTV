...
const AIRecommendConfig = ({
  ...
  const handleSave = async () => {
    ...
    setIsLoading(true);
    try {
      await api.post('/api/admin/ai-recommend', aiSettings);

      ToastManager.success('AI推荐配置保存成功');
      await refreshConfig();
    } catch (err) {
      ToastManager.error(err instanceof Error ? err.message : '保存失败');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleTest = async () => {
    ...
    setIsLoading(true);
    try {
      const result = await api.post('/api/admin/ai-recommend/test', {
